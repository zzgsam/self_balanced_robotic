...
#	print(gpio_test.pyReadDuration())
	i=0
	period_test=[0 for j in range(12)]
	pwm_value={}
	for i in range(100):
		p.ChangeDutyCycle(i+1)
		for k in range(12):
			period_test[k]=gpio_test.pyReadPeriod()
			sleep(1)
		pwm_value[i+1]=(sum(period_test)-period_test[0]-period_test[1])/10 	
		print(pwm_value[i+1])	
	with open("./pwm.txt", 'a+') as f:
		for i in pwm_value:
			f.write("%d %s\n"%(i,pwm_value[i]))
